...
| **Studio** | Public actor (`handle`). Person-owned or Organization-owned. Table `profiles`. API `profileId`. |
| **Members** | Invited collaborators, seats, section access. Billing stays on the studio row. |
| **Follow** | Audience for shareable rows (`members` \| `followers` \| `public`) |
...
- Apps are not separate SaaS products: they share auth and `profileId`.
- Follow is not membership. Public pages are not the studio shell.
